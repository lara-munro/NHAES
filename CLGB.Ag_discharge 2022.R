...
# Read tables from 2022 for silver body HOBO
...
# Remove data from before the logger was installed in the stilling well
# HOBO was only installed on 8/8/2022 at 10:15
sta <- sta[-c(1:574),]
...
plot(dat$DateTime.EST, dat$Q.m3s, type = "l",
     ylab = "Q (m3/s)", xlab = "Date 2022")
plot(dat$DateTime.EST, dat$Q.m3s, type = "l", log = "y",
     ylab = "Q (m3/s)", xlab = "Date 2022")

dat2 <- dat
dat$DateTime.EST <- as.character(format(dat$DateTime.EST))
write.csv(dat, paste0(dataloc,"/CLGB.AG/data/ratingCurve/CLGB.AG_2022_DISCHARGE.csv"), row.names = F)
write.csv(dat, paste0(dataloc,"/CLGB.AG/data/stage/y2022/finalData/CLGB.AG_2022_DISCHARGE.csv"), row.names = F)
dat <- dat2


# Import Q measurements ---------------------------------------------------

dat <- read.csv(paste0(dataloc,"/CLGB.AG/data/stage/y2022/finalData/CLGB.AG_2022_DISCHARGE.csv"))
