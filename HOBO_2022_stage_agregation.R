...
# 2. Read old HOBO location ------------------------------------------------

# Read tables from 2022 for old HOBO
staloc <- paste0(dataloc, "CLGB.AG/data/stage/y2022/rawData/oldHOBO/")
...
sta <- sta %>%
  rename(DateTime = Date.Time..GMT.05.00,
         pres.kpa.old = Abs.Pres..kPa..LGR.S.N..20341743..SEN.S.N..20341743..LBL..PRESSURE.,
         temp.C.old = Temp...C..LGR.S.N..20341743..SEN.S.N..20341743..LBL..TEMP.)
...
# 3. Read new HOBO location ------------------------------------------------

# Read tables from 2022 for old HOBO
staloc <- paste0(dataloc, "CLGB.AG/data/stage/y2022/rawData/newHOBO/")
flist <- list.files(staloc, pattern = "*.csv")
flist
...
sta <- sta %>%
  rename(DateTime = Date.Time..GMT.05.00,
         pres.kpa.new = Abs.Pres..kPa..LGR.S.N..1303191..SEN.S.N..1303191.,
         temp.C.new = Temp...C..LGR.S.N..1303191..SEN.S.N..1303191.)
sta$DateTime <- as.POSIXct(sta$DateTime, tz = "EST",
                           format = "%m/%d/%y %I:%M:%S %p")
sta <- na.omit(sta)
...
# water pressure
dat$watPres.kPa.old <- dat$pres.kpa.old - dat$pres.kpa.baro
dat$watPres.kPa.new <- dat$pres.kpa.new - dat$pres.kpa.baro


# Fluid density calculation (from Reese LeVea)
dat$density.old = (999.83952 + (16.945176 * dat$temp.C.old) - 
                        (7.9870401e-03 * dat$temp.C.old**2) - 
                        (46.170461e-06 * dat$temp.C.old**3) + 
                        (105.56302e-09 * dat$temp.C.old**4) - 
                        (280.54253e-12 * dat$temp.C.old**5)) /
  (1 + 16.879850e-03 * dat$temp.C.old) # density in kg/m3

dat$density.new = (999.83952 + (16.945176 * dat$temp.C.new) - 
                       (7.9870401e-03 * dat$temp.C.new**2) - 
                       (46.170461e-06 * dat$temp.C.new**3) + 
                       (105.56302e-09 * dat$temp.C.new**4) - 
                       (280.54253e-12 * dat$temp.C.new**5)) /
  (1 + 16.879850e-03 * dat$temp.C.new) # density in kg/m3

# Convert density from kg/m3 to lb/ft^3
dat$density.lbft.old <- 0.0624279606 * dat$density.old
dat$density.lbft.new <- 0.0624279606 * dat$density.new

# Convert pressure to density-dependent fluid depth array

dat$depth.m.old = (FEET_TO_METERS) * (KPA_TO_PSI * PSI_TO_PSF * dat$watPres.kPa.old) / dat$density.lbft.old
dat$depth.m.new = (FEET_TO_METERS) * (KPA_TO_PSI * PSI_TO_PSF * dat$watPres.kPa.new) / dat$density.lbft.new
